// MyMeshTiledTest.cpp
// -- test of textured mesh derived from TiledMesh class.
// cs200 2021.10
//
// To compile from the Visual Studio command prompt:
//   cl /EHsc MyTiledMeshTest.cpp MyTiledMesh.cpp Affine.lib
//   Texture.lib TextureRender.lib opengl32.lib glew32.lib SDL2.lib
//   SDL2main.lib /link /subsystem:console
//
// Note: the file 'tile.bmp' is assumed to exist.
...
const glm::vec4 O = cs200::point(0, 0), BLACK(0, 0, 0, 1);
...
Client::Client(void) {
  ...
  try {
    texture = new cs200::Bitmap("tile.bmp");
    cs200::reverseRGB(*texture);
  } catch (exception &e) {
    cout << "failed to open 'tile.bmp'" << endl;
    cout << e.what() << endl;
  ...
  for (int i = 0; i < COUNT; ++i) {
    for (int j = 0; j < COUNT; ++j) {
      float sign_x = (i % 2 == 0) ? 1 : -1, sign_y = (j % 2 == 0) ? 1 : -1;
      glm::mat4 obj2world = cs200::translate(cs200::vector(
                                (i + 0.5f) * dx - 1, (j + 0.5f) * dy - 1)) *
...
int main(int, char *[]) {
  ...
  const char *title = "CS 200: MyTiledMesh Test";
